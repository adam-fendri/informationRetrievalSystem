...
def main(true_path, fake_path):
    ...
    while True:
        ...
        # Consider only the top 10 retrieved documents
        top_indices = ranked_indices[:10]
        retrieved_labels = [labels[idx] for idx in top_indices]  # Labels of top 10 retrieved documents
        true_relevants = sum(retrieved_labels)  # Count of relevant documents in top 10
        
        # Metrics calculation
        precision = true_relevants / 10  # Out of 10 retrieved documents
        recall = true_relevants / len([label for label in retrieved_labels if label == 1]) if true_relevants > 0 else 0
